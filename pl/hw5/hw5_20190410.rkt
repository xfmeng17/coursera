  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([cls (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? cls)
               (let* ([fundata (closure-fun cls)]
                      [bodyenv (cons (cons (fun-formal fundata) arg) (closure-env cls))]
                      [bodyenv (if (fun-nameopt fundata)
                                   (cons (cons (fun-nameopt fundata) cls) bodyenv)
                                   bodyenv)])
                 (eval-under-env (fun-body fundata) bodyenv))
               (error "MUPL function call with non-function")))]
